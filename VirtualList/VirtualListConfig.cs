...
namespace VirtualList;
    ...
    public struct VirtualListConfig
    {
        ...
        /// <summary>是否沿主轴反向排列（类似 CSS RowReversed / ColumnReversed：起始元素置于主轴末端）。
        /// 仅反转主轴顺序与滚动原点；交叉轴列顺序不受影响。可在运行时经 <c>ApplyConfig</c> 改变。</summary>
        public bool Reversed;

        /// <summary>初始滚动锚点（列表初始 / 重置时的滚动位置；正交于反向排列）。
        /// 可在运行时经 <c>ApplyConfig</c> / <c>SetInitialAnchor</c> 改变。</summary>
